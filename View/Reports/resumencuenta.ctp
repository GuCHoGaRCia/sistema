...
<html lang="es-419">
    ...
    <body>
        <img src="/sistema/img/print2.png" id="print" style='cursor:pointer;position:absolute;right:0' onclick="hide();window.print();show()">
        <?php
        // si viene desde "colaimpresiones", los datos estan en $data, sino en $data['Colaimpresione']['data']
        ...
        foreach ($data['prop'] as $p) {
            if ($p['id'] == $propietario) {
                ...
                reverso($p, $data, $client, $consorcio, $info, $prefijo, $esinicial, $this->webroot, $muestraIconoDetalleCobranza, $codpagelect);
...
function reverso($p, $data, $client, $consorcio, $info, $prefijo, $esinicial, $webroot, $muestraIconoDetalleCobranza, $cpe) {
    ...
    $sa = $esinicial || (isset($data['saldosanteriores'][$p['id']]['capital']) && $data['saldosanteriores'][$p['id']]['capital'] < 0) ? $saldant : $sant;
    ?>
    <table width=750 valign=top cellspacing=0 <?= $formato ?> class="box-table-b" align="center">
        <tr>
            <td class="pri"><b>Saldo anterior</b></td>
            <td colspan="2" style='text-align:center'>(Capital: <?= money(isset($data['saldosanteriores'][$p['id']]['capital']) ? $data['saldosanteriores'][$p['id']]['capital'] : 0) ?>
                | Inter&eacute;s: <?= money(isset($data['saldosanteriores'][$p['id']]['interes']) ? $data['saldosanteriores'][$p['id']]['interes'] : 0) ?>)</td>
            <td colspan=3 class="right"><?php echo money($sa) ?></td>
        </tr>
        <tr>
        ...
        foreach ($data['cobranzas'] as $v) {
            if ($v['Cobranza']['propietario_id'] == $p['id']) {
                ...
                <tr>
                    <td class="pri"></td>
                    <td colspan=2 style='text-align:center'>Su pago: 
                        <?php
                        echo date("d/m/Y", strtotime($v['Cobranza']['fecha'])) . ($v['Cobranzatipoliquidacione']['solocapital'] ? ' (solo capital)' : '') . (isset($v['Cobranza']['numero']) ? " - Recibo Oficial N&ordm; " . $v['Cobranza']['numero'] : '');
                        if ($muestraIconoDetalleCobranza) {
                            echo "&nbsp;<img src='" . $webroot . "img/icon-info.png' onclick='openInNewTab(" . $v['Cobranza']['id'] . ")' style='cursor:pointer' />";
                        }
                        ?>
                    </td>
                    <?php
                    ...
                    <script>
                        if (window.jQuery) {
                            $(function () {
                                var dialog2 = $("#rc2").dialog({
                                    autoOpen: false, height: "400", width: "800", maxWidth: "800",
                        ...
                        function openInNewTab(id) {
                            window.open("<?= $webroot ?>Cobranzas/view/" + id);
                        }

                    </script>
                    <?= "<div id='rc2' style='display:none;margin:0 auto;background:#fff;z-index:1000000'></div>"; // es el div para el RC    ?>
                    <?php
                }
                ?>
    ...
    foreach ($data['ajustes'] as $v) {
        if ($v['Ajuste']['propietario_id'] == $p['id']) {
            ...
            <tr>
                <td class="pri"></td>
                <td colspan=2 style='text-align:center'>Ajuste: <?= date("d/m/Y", strtotime($v['Ajuste']['fecha'])) . ($v['Ajustetipoliquidacione']['solocapital'] ? ' (solo capital)' : '') ?></td>
    ...
    <tr>
        ...
        <td colspan="2" style='text-align:center'>(Capital: <?= money($data['remanentes'][$p['id']]['capital']) ?> | Inter&eacute;s: <?= money($data['remanentes'][$p['id']]['interes']) ?>)</td>
        <td colspan=3 class="right">
            ...
            if ($sr < 0) {// si el $sr es negativo (saldo a favor), tengo q sacarle el redondeo. No poner $sf < 0 || porq sino los q tienen saldo final negativo no le resta RANT (esta en cero)
    ...
    if (isset($data['totales'][$p['id']]['coefgen'])) { // tiene gastos generales prorrateados
        foreach ($data['totales'][$p['id']]['coefgen'] as $k => $v) {
            ...
            if (isset($data['descripcioncoeficientes'][$k]) && isset($v['val'])) {
                ...
                <tr>
                    <td class="pri"></td>
                    <td colspan=2 style='text-align:center'><?= h($data['descripcioncoeficientes'][$k]) ?> <?= $v['val'] > 0 ? "(" . money($v['tot'] * 100 / $v['val']) . ")" : '' ?> <?= $v['val'] ?>%</td>
    ...
    if (isset($data['totales'][$p['id']]['detalle'])) { // tiene gastos particulares propios del propietario
        foreach ($data['totales'][$p['id']]['detalle'] as $v) {
            ...
            <tr>
                <td class="pri"></td>
                <td colspan=2 style='text-align:center'><?= h($v['descripcion']) ?></td>
        ...
        foreach ($data['totales'][$p['id']]['coefpar'] as $v) {
            ...
            foreach ($v['detalle'] as $w) {
                if (isset($v['val']) && $v['val'] > 0) {// no muestro los q esten en cero
                    ...
                    <tr>
                        <td class="pri"></td>
                        <td colspan=2 style='text-align:center'><?= h($w['descripcion']) ?> (<?= money($w['total']) ?>) <?= $v['val'] ?>%</td>
    ...
    <tr class="totales">
        ...
        <td colspan="2" style='text-align:center'><?= ($sf > 0 && !isset($data['consorcio']['imprimeimportebanco'])) || ($sf > 0 && (isset($data['consorcio']['imprimeimportebanco']) && $data['consorcio']['imprimeimportebanco'])) ? "IMPORTE DEPOSITO BANCARIO: " . money($sf + (($p['code'] / 100) - intval(($p['code'] / 100)))) : '' ?></td>
        <td colspan=3 class="right">
            <?php
            echo money($sf);
            ?>
    ...
    <?php
    if (isset($data['plataforma']['plataformasdepago_id']) && $data['plataforma']['plataformasdepago_id'] == 1 && strlen($cpe) == 14) { // es PLAPSA, muestro el boton de pago
        /*
          ?>
          <tr>
          <td colspan="6">
          <a title="Pagar Expensa" target="_blank" rel="noreferrer noopener" href="https://portal.expensaspagas.online/?code=<?= h($cpe) ?>&name=&email=&view=full"><img src="/sistema/img/pagarexpensa.png" /></a>
          </td>
          </tr>
          <?php
         */
