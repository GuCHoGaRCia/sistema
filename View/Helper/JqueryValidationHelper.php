...
class JqueryValidationHelper extends AppHelper {
  ...
  public $options = array(
	  'map' => array(
		  ...
		  '__errorClass__' => 'help-block2',
		  '__hilightClass__' => 'form-error ib',
		  '__closestSelector__' => '.control-group',
		  '__closestErrorClass__' => 'error'
	  )
  );
  // $('.help-block').css('display', 'inline-block'); 
  //agregue en $js esta linea sino cuando modificaba un campo con error, el error se mostraba abajo si volvia a ocurrir
  private $js = "$(document).ready(function(){ $('__formSelector__').each( function(index) { $.metadata.setType('attr','data-validate');$(this).validate({'errorElement': '__errorElement__','errorClass': '__errorClass__','highlight': function(element,errorClass) { $(element).siblings('__errorClass__').remove();$('.help-block2').css('display', 'inline-block');$(element).addClass('__hilightClass__').closest('__closestSelector__').addClass('__closestErrorClass__');},'unhighlight': function(element,errorClass) { $(element).siblings('__errorClass__').remove();$(element).removeClass('__hilightClass__').closest('__closestSelector__').removeClass('__closestErrorClass__')}})})})";
  ...
  public function createVertical($model = null, $options = array()) {
	...
	$this->options['map']['__errorClass__'] = 'help-block2';
	return $this->Form->create($model, $options);
  }
  ...
  public function input($fieldName, $options = array()) {
	if (!isset($options['error']['attributes']['class'])) {
	  @$options['error']['attributes']['class'] = $this->map['error-class']??null;
	}
  ...
  private function jquery_validate_notEmpty($model, $field, $params, $msg) {
	...
	$response['messages']['required'] = "hola";
