<?php
//debug($coeficientes);die;
?>
<!DOCTYPE html>
<html lang="es-419">
    <head>
        <title>Composici&oacute;n de saldos Particulares Coeficientes - <?= h($consorcio['Consorcio']['name'] . " - " . $info['Liquidation']['periodo']) ?></title>
    ...
    <body>
        ...
        detalle($data, $cuentasinfotemp, $data['gastosinfo'], $consorcio['interes'], $esinicial, $coeficientes);
                ...
                <tr>
                    ...
                    <td align="left">
                        ...
                        <p style="border-top:2px solid #9baff1;text-align:center;line-height:7px;"><br>
                            ...
                            <br><br>SA: Saldo Anterior, RANT: Redondeo Anterior, COB: Cobranzas, AJ: Ajustes, SR: Saldo Remanente, <br><br>GP: Gastos Particulares, 
        ...
        function detalle($data, $cuentasinfo, $gastosinfo, $consorciointeres, $esinicial, $coeficientes) {
            ...
            <table valign=top cellspacing=0 <?= $formato ?> class="box-table-b" align="center" id="floatth">
                <thead>
                    <tr>
                        <th class="totales" style="width:auto"><b>Unidad (<?= count($data['prop']) ?>)</b></th>
                        <?php
                        $gpart = $totalporcentajes = [];
                ...
                foreach ($data['prop'] as $p) {
                    ...
                    foreach ($data['descripcioncoeficientes'] as $k => $v) { // los %
                        $totalporcentajes[$k] += isset($data['totales'][$p['id']]['coefpar'][$k]['val']) ? $data['totales'][$p['id']]['coefpar'][$k]['val'] : 0;
                        echo "<td style='text-align:right;width:auto'>" . (isset($data['totales'][$p['id']]['coefpar'][$k]['val']) ? $data['totales'][$p['id']]['coefpar'][$k]['val'] : 0) . "</td>";
                    }
                ...
                // muestro los totales
                echo "<tr style='font-weight:bold'><td style='width:100px;text-align:center'>TOTALES</td>";
                foreach ($data['descripcioncoeficientes'] as $k => $v) { // los %
