<?php
// si es alguno de nosotros
$si = in_array($_SESSION['Auth']['User']['username'], ['rcasco', 'mmazzei', 'ecano', 'mcasalderrey']);
?>
<div class="liquidationsTypes index">
    <h2><?php echo __('Tipos de liquidaciones'); ?></h2>
    <?php echo $this->element('toolbar', array('pagecount' => true, 'pagesearch' => true, 'pagenew' => $si, 'model' => 'LiquidationsType')); ?>
        ...
        <tbody>
            ...
            foreach ($liquidationsTypes as $liquidationsType):
                ...
                <tr<?php echo $class; ?>>
                    ...
                    if ($si) {
                        ?>
                        <td class="center"><span class="prefijo" data-value="<?php echo h($liquidationsType['LiquidationsType']['prefijo']) ?>" data-pk="<?php echo h($liquidationsType['LiquidationsType']['id']) ?>"><?php echo h($liquidationsType['LiquidationsType']['prefijo']) ?></span>&nbsp;</td>
                        <?php
                    } else {
                        ?>
                    ...
                    <td class="acciones" style="width:auto">
                        ...
                        //echo $this->Form->postLink($this->Html->image('delete.png', array('alt' => __('Eliminar'), 'title' => __('Eliminar'))), array('action' => 'delete', $liquidationsType['LiquidationsType']['id']), array('escapeTitle' => false), __('Eliminar el dato # %s?', $liquidationsType['LiquidationsType']['name']));    
                        ?>
                    </td>
                    <td class="borde_tabla"></td>
                </tr>
...
<script>
    $(document).ready(function () {
        ...
        $('.prefijo').editable({type: 'text', name: 'prefijo', success: function (n, r) {
                if (n) {
                    return n
                }
            }, url: '<?php echo $this->webroot; ?>LiquidationsTypes/editar', placement: 'right'});
