...
function detalle($data, $rubrosinfo, $gastosinfo) {
    ...
    <table valign=top cellspacing=0 <?= $formato ?> class="box-table-b" align="center">
        ...
        foreach ($rubrosinfo as $k => $v) {
            ...
            $linearubro = "";
            $linearubro .= "<tr>";
            $linearubro .= "<td class='rubrotitle' colspan='" . (count($data['descripcioncoeficientes']) + 2) . "'><b>$rubrocount - " . h($v) . " " . round(((!empty($totalrubroporcentaje[$k]) ? $totalrubroporcentaje[$k] : 0) * 100 / ($totaldeporcentaje == 0 ? 1 : $totaldeporcentaje)), 2) . "%</b></td>";
            $linearubro .= "</tr>";
            if (isset($gastosinfo) && count($gastosinfo) > 0) {
                foreach ($gastosinfo as $l => $m) {
                    $totalgasto = 0;
