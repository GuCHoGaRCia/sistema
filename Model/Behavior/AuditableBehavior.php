...
class AuditableBehavior extends \ModelBehavior {
    ...
    public function setup(Model $Model, $settings = []) {
        ...
        if (!isset($this->settings[$Model->alias])) {
            ...
            $this->settings[$Model->alias]['habtm'] = [];
            if (!isset($settings['habtm'])) {
                $this->settings[$Model->alias]['habtm'] = count($Model->hasAndBelongsToMany) > 0 ? array_keys($Model->hasAndBelongsToMany) : [];
            }
        }
    ...
    public function afterSave(Model $Model, $created, $options = []) {
        ...
        $data = array(
            'Audit' => array(
                ...
                'client_id' => isset($source['client_id']) ? $source['client_id'] : 0,
            ),
        );
        ...
        if ($created || count($updates)) {
            /*
             * AGREGUE ESTO PORQUE NO TOMA EL $Model->bindModel() ANTERIOR, ANDA A SABER PORQUE!!! HDP
             */
    ...
    protected function _isAuditLogModel(Model $Model) {
        return $Model->name === 'Audit' || $Model->name === 'AuditDelta' || $Model->name === 'Aviso' || $Model->name === 'Avisosqueue' || $Model->name === 'SaldosCierre' || $Model->name === 'Saldoscajabanco';
    }
