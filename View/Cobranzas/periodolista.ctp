...
<div class="coeficientesPropietarios index" id='tabla'>
    <?php echo $this->Form->create('Cobranza', array('class' => 'jquery-validation', 'url' => ['controller' => 'Cobranzas', 'action' => 'periodo'], 'id' => 'formperiodo')); ?>
                ...
                <tr<?php echo $class; ?>>
                    ...
                    foreach ($lt as $r => $s1) {
                        ...
                        <td class='center inline'>
                            <?php
                            echo $this->JqueryValidation->input('f_' . $k . '_' . $r, ['label' => __('Fecha pago'), 'id' => 'f_' . $k . '_' . $r, 'type' => 'text', 'style' => 'width:90px', /* 'readonly' => 'readonly', */ 'class' => 'dp' . " in_" . $r]);
    ...
    $opt = [$fdp['Efectivo'] => '&nbsp;Efectivo']; //, 
    echo $this->JqueryValidation->input('consorcio_id', ['type' => 'hidden', 'value' => $cid]); // consorcio_id
    $cuenta = "";
    if (!empty($cb)) {
        $opt += [$fdp['Transferencia'] => '&nbsp;Transferencia', $fdp['Interdepósito'] => '&nbsp;Interdep&oacute;sito'];
        $cuenta = $this->JqueryValidation->input('bancoscuenta_id', ['type' => 'select', 'label' => false, 'class' => 's2', 'options' => $cb]);
    }
    echo $this->Form->radio('fdp', $opt, ['legend' => false, 'value' => $fdp['Efectivo'], 'label' => false, 'style' => 'display:inline;margin-left:30px']);
    echo "</div><br>";
    echo "<div id='listabancos' style='display:none;margin-left:120px'>$cuenta</div>";
...
<?php echo '<br>' . $this->Html->link(__('Cancelar'), ['controller' => 'cobranzas', 'action' => 'periodo'], [], __('Desea cancelar?')); ?>
<script>
<?php
echo "var totals = [";
$count = count($lt);
...
?>
    ...
    $(document).ready(function () {
        ...
        $('input[type=radio][name="data[Cobranza][fdp]"]').change(function () {
            if (this.value === 'E') {
                $("#listabancos").hide();
            } else {
                $("#listabancos").show();
    ...
    $(document).on('click', '#guardar', function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?= $this->webroot ?>Cobranzas/periodo",
            ...
            try {
                ...
                if (obj.e === 1) {
                    ...
                    window.location.replace("<?= $this->webroot ?>Cobranzas/periodo");
                }
            } catch (err) {
            }
