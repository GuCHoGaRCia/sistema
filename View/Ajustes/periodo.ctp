<div class="cobranzas form">
    <fieldset>
        <h2><?php echo __('Agregar Ajuste por Período'); ?></h2>
        <?php
...
<script>
    ...
    function saldo(id) {
        ...
        $.ajax({type: "POST", url: "<?= $this->webroot ?>SaldosCierres/getSaldosTipoLiquidacionPropietarios", cache: false, data: {c: id, s: 'Ajustes'}}).done(function (msg) {
            $("#formas").html(msg);
    ...
    function recargar() {
        if ($("#consorcio_id").val() !== "") {
            window.location.href = "<?= $this->webroot ?>Ajustes/periodo/" + $("#consorcio_id").val();
        }
    }
</script>