<div class="reparacionesactualizaciones form">
    ...
    <fieldset>
        <h3><?php echo __('Editar última actualización') . h(" " . $reparaciones['Consorcio']['name'] . " - " . ($reparaciones['Propietario']['name2'] ?? '')) ?></h3>
        <?php
        $client_id = $this->Functions->_encryptURL($reparaciones['Consorcio']['client_id']);
        echo $this->JqueryValidation->input('c', ['type' => 'hidden', 'value' => $client_id]);
        $r = $reparaciones['Reparacionesactualizacione'][0]; // es la ultima actualizacion
        echo $this->JqueryValidation->input('reparacioneactualizacione_id', ['type' => 'hidden', 'value' => $r['id']]);
        echo $this->JqueryValidation->input('reparacione_id', ['type' => 'hidden', 'value' => $reparaciones['Reparacione']['id']]);
        //debug($supervisor);
        if (isset($supervisor)) {
            foreach ($supervisor as $supk => $supv) {
                echo $this->JqueryValidation->input('supervisor', ['type' => 'hidden', 'value' => $supk]);
            }
        }
        echo $this->JqueryValidation->input('consorcio_id', ['type' => 'hidden', 'value' => $reparaciones['Reparacione']['consorcio_id']]);
        $estadoanterior = empty($reparaciones['Reparacionesactualizacione']) ? $reparaciones['Reparacione']['reparacionesestado_id'] : (isset($reparaciones['Reparacionesactualizacione'][1]['reparacionesestado_id']) ? $reparaciones['Reparacionesactualizacione'][1]['reparacionesestado_id'] : $reparaciones['Reparacionesactualizacione'][0]['reparacionesestado_id']);
        echo "<div class='inline'>";
        echo $this->JqueryValidation->input('reparacionesestado_id', ['label' => __('Estado') . ' (Anterior: ' . $reparacionesestados[$estadoanterior] . ')', 'selected' => $r['reparacionesestado_id']]);
        echo $this->JqueryValidation->input('fecha', ['label' => __('Fecha') . ' *', 'type' => 'text', 'class' => 'dp', 'style' => 'width:95px', 'value' => date("d/m/Y", strtotime($r['fecha']))]);
        echo $this->JqueryValidation->input('concepto', ['label' => __('Concepto') . ' *', 'value' => $r['concepto']]);
        echo "</div>";
        echo $this->JqueryValidation->input('observaciones', ['label' => __('Observaciones'), 'class' => 'ckeditor', 'value' => $r['observaciones']]);
        ...
        if (isset($reparaciones['Reparacionesactualizacione'][0]['Reparacionesactualizacionesadjunto']) && !empty($reparaciones['Reparacionesactualizacione'][0]['Reparacionesactualizacionesadjunto'])) {
            $cad = "&nbsp;<u>Adjuntos:</u> ";
            $hay = false;
            foreach ($reparaciones['Reparacionesactualizacione'][0]['Reparacionesactualizacionesadjunto'] as $r => $s) {
                $hay = true;
                $cad .= "<span id='adj" . $s['id'] . "'>" . $this->Html->link(h($s['titulo']), array('controller' => 'Reparacionesactualizacionesadjuntos', 'action' => 'download', $this->Functions->_encryptURL($s['ruta']), $client_id));
                $cad .= "&nbsp;" . $this->Html->image('drop.png', ['alt' => __('Eliminar adjunto'), 'title' => __('Eliminar adjunto'), 'data-name' => h($s['titulo']), 'class' => 'propimgdel', 'data-id' => $s['id'], 'style' => 'cursor:pointer']);
                $cad .= " ||&nbsp;&nbsp;</span>";
            }
            echo $hay ? substr($cad, 0, -21) . "<br>" : '';
        }
        echo "</div>";
        //if (!isset($supervisor)) {//si no esta seteada, esta logueado en el Sistema. Sino, es un supervisor y no tengo q permitirle asignar/finalizar supervisores o proveedores
        $finalizar = $asignar = [];
        //debug($reparaciones);
        foreach ($reparaciones['Reparacionesactualizacione'] as $v) {
            foreach ($v['Reparacionesactualizacionessupervisore'] as $w) {
                ...
                if ($w['finalizado']) {
                    ...
                    //}
                    if (isset($finalizar[$w['reparacionessupervisore_id']])) {
                        unset($finalizar[$w['reparacionessupervisore_id']]);
                    }
                } else {
                    //if (!isset($asignar[$w['reparacionessupervisore_id']])) {
                    $finalizar[$w['reparacionessupervisore_id']] = $reparacionessupervisores[$w['reparacionessupervisore_id']];
                    if (isset($asignar[$w['reparacionessupervisore_id']])) {
                        unset($asignar[$w['reparacionessupervisore_id']]);
                    }
                    //}
                }
            }
...
<?php /* deshabilito los input q no sirven */ ?>
        ...
        $.ajax({
            type: "POST",
            url: "<?= $this->webroot ?>Reparacionesactualizaciones/edit",
            data: fd,
            contentType: false,
            processData: false
        }).done(function (msg) {
...
<script>
    ...
    $(document).on('click', '.propimgdel', function () {
        if (confirm('<?= __("Desea eliminar el adjunto ") ?>"' + $(this).data('name') + '"')) {
            var id = $(this).data('id');
            $.ajax({type: "POST", url: "<?= $this->webroot ?>Reparacionesactualizaciones/delAdjunto", cache: false, data: {id: id, c: '<?= $client_id ?>'}}).done(function (msg) {
                if (msg === "true") {
